    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col justify-between gap-4 border-b border-zinc-100 dark:border-zinc-800 pb-8 sm:flex-row sm:items-end">
                <div>
                    <h1 className="text-3xl tracking-tighter uppercase text-black dark:text-white lg:text-5xl">Orders</h1>
                    <p className="mt-2 text-xs uppercase tracking-widest text-zinc-500">
                        Manage and fulfill customer orders
                    </p>
                </div>
                <div className="flex gap-2">
                    <button onClick={fetchOrders} className="bg-black px-6 py-3 text-[10px] uppercase tracking-widest text-white hover:bg-zinc-800 dark:bg-white dark:text-black hover:opacity-80 transition-opacity">
                        Refresh
                    </button>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
                            <tr>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-zinc-500">ID</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-zinc-500">Customer</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-zinc-500">Date</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-zinc-500">Total</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-zinc-500">Payment</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-zinc-500">Status</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-zinc-500 text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={7} className="px-6 py-20 text-center text-zinc-500 uppercase tracking-widest text-xs">No orders found</td>
                                </tr>
                            ) : (
                                orders.map((order) => (
                                    <tr key={order.id} className="group hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-black dark:text-white uppercase tracking-tight text-xs">{order.customerName}</td>
                                        ...
                                        <td className="px-6 py-4 text-black dark:text-white">₹{order.total_amount}</td>
                                        <td className="px-6 py-4">
                                            <span className="inline-flex items-center gap-1.5 rounded bg-zinc-100 dark:bg-zinc-800 px-2 py-1 text-[10px] uppercase tracking-widest text-zinc-600 dark:text-zinc-400">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <button
                                                ...
                                                className="inline-flex items-center gap-1 text-[10px] uppercase tracking-widest text-zinc-400 hover:text-black dark:hover:text-white transition-colors border p-2 border-zinc-200 dark:border-zinc-800 hover:border-black dark:hover:border-white"
                                            >
                                                Details
                                            </button>
                                        </td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="w-full max-w-2xl bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 shadow-2xl max-h-[90vh] overflow-y-auto">
                        <div className="border-b border-zinc-100 dark:border-zinc-800 p-6 flex justify-between items-start sticky top-0 bg-white dark:bg-black z-10">
                            <div>
                                <h2 className="text-2xl tracking-tighter uppercase">Order #{selectedOrder.id.split('-')[0]}</h2>
                        ...
                        <div className="p-6 space-y-8">
                            ...
                            <div className="grid grid-cols-2 gap-8">
                                <div>
                                    <h3 className="text-[10px] uppercase tracking-widest text-zinc-400 mb-2">Customer</h3>
                                    <div className="flex items-center gap-3">
                                        <div className="h-10 w-10 bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center">
                                            <User className="h-4 w-4 text-zinc-500" />
                                        </div>
                                        <div>
                                            <p className="text-sm uppercase">{selectedOrder.customerName}</p>
                                            <p className="text-xs text-zinc-500">ID: {selectedOrder.user_id}</p>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <h3 className="text-[10px] uppercase tracking-widest text-zinc-400 mb-2">Update Status</h3>
                                    <div className="flex flex-wrap gap-2">
                                        {['pending', 'processing', 'shipped', 'delivered', 'cancelled'].map(s => (
                                            <button
                                                key={s}
                                                onClick={() => updateStatus(s)}
                                                className={`px-3 py-1.5 text-[10px] uppercase tracking-widest border transition-colors ${selectedOrder.status === s
                            ...
                            {/* Items */}
                            <div>
                                <h3 className="text-[10px] uppercase tracking-widest text-zinc-400 mb-4">Items Ordered</h3>
                                                    ...
                                                    <div className="flex justify-between items-start">
                                                        <div>
                                                            ...
                                                            <div className="flex gap-3 mt-1">
                                                                <p className="text-[10px] text-zinc-500 uppercase">Size: {item.size || 'N/A'}</p>
                                                                <p className="text-[10px] text-zinc-500 uppercase">Mat: {item.material || 'N/A'}</p>
                                                            </div>
                                                        </div>
                            ...
                            <div className="border-t border-zinc-100 dark:border-zinc-800 pt-6 space-y-2">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="">₹{selectedOrder.total_amount}</span>
                                </div>
                                <div className="flex justify-between text-xl uppercase tracking-tighter mt-4">
...
function StatusBadge({ status }: { status: string }) {
    ...
    return (
        <span className={`inline-flex items-center rounded px-2.5 py-0.5 text-[10px] uppercase tracking-widest ${cn}`}>
            {status}
        </span>
    );
}
